...
plot( main="Performance of Connectivity on (10-node) Full Forests, 1000 runs"
    , xrange, yrange 
    , xlab = "" , ylab = "" , lwd  = 0
    , cex.lab=1.75, cex.axis=1.75, cex.main=1.5, cex.sub=1.75)
title(ylab="running time (m seconds)", line=2.5, cex.lab=1.75)
# ylab=expression(paste("running time (" ~ m ~ "seconds)"))
title(xlab="number of nodes (sampling every 500 medians)", line=2.5, cex.lab=1.75)
...
legText = c("10-node,1000 runs,conn=False","10-node,1000 runs,conn=True")
legCol  = c("blue","salmon4") 
legLTY  = c(1,1) 
legLWD  = c(1,1)
legPCH  = c(13,5)
